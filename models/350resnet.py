...
class LearningRateMonitor(Callback):
    ...
    def __init__(self): super().__init__(); self.last_lr = {}
    def on_train_epoch_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        if not trainer.optimizers: return
        for i, optimizer in enumerate(trainer.optimizers):
            if not optimizer.param_groups: continue
            current_lr = optimizer.param_groups[0]['lr']; optimizer_key = f"optimizer_{i}_group_0"
            ...
            if last_lr_group is not None and not np.isclose(current_lr, last_lr_group, rtol=1e-6, atol=1e-9):
                print("\n" + "="*60 + f"\nEpoch {trainer.current_epoch}: LR for optimizer {i} changed from {last_lr_group:.4e} to {current_lr:.4e}\n" + "="*60 + "\n")
...
class KelpSegmentationModel(pl.LightningModule):
    def __init__(self, target_size=(350, 350), learning_rate=1e-4, backbone_name="resnet18"):
        super().__init__(); self.save_hyperparameters(); self.target_size = target_size
        if self.hparams.backbone_name == "resnet18": base_encoder = torchvision.models.resnet18(weights=ResNet18_Weights.DEFAULT); encoder_out_channels = 512
        elif self.hparams.backbone_name == "resnet34": base_encoder = torchvision.models.resnet34(weights=ResNet34_Weights.DEFAULT); encoder_out_channels = 512
    ...
    def training_step(self, batch, batch_idx): images, masks = batch; logits = self(images); loss = self.criterion(logits, masks); self.log('train_loss', loss, on_step=True, on_epoch=True, prog_bar=True, logger=True); return loss
    def validation_step(self, batch, batch_idx): images, masks = batch; logits = self(images); loss = self.criterion(logits, masks); preds_prob = torch.sigmoid(logits); preds_binary = (preds_prob > 0.5); iou = self.val_iou(preds_binary, masks.int()); self.log('val_loss', loss, on_step=False, on_epoch=True, prog_bar=True, logger=True); self.log('val_iou', iou, on_step=False, on_epoch=True, prog_bar=True, logger=True); return loss
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=self.hparams.learning_rate) # Changed back to Adam, use AdamW if preferred
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=MAX_EPOCHS, eta_min=COSINE_ETA_MIN)
...
class KelpDataset(torch.utils.data.Dataset):
    def __init__(self, satellite_paths: List[str], mask_paths: List[str],
        ...
        self.satellite_paths=satellite_paths; self.mask_paths=mask_paths; self.transforms=transforms; self.apply_noise=apply_noise; self.noise_level=noise_level
    ...
    def load_image(self, filename: str, is_mask: bool = False):
        try:
            img = tiff.imread(filename); assert img is not None
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.transforms: augmented = self.transforms(image=sat_img_np, mask=mask_np); sat_img_np, mask_np = augmented['image'], augmented['mask']
            sat_tensor = torch.from_numpy(sat_img_np.transpose(2, 0, 1)); mask_tensor = torch.from_numpy(mask_np).unsqueeze(0).float()
            if self.apply_noise: noise = torch.randn_like(sat_tensor[:5, :, :]) * self.noise_level; sat_tensor[:5, :, :] += noise
...
def main():
    ...
    # ============================================================================
    # ADDED: Conditional Augmentation Logic
    # ============================================================================
    if APPLY_AUGMENTATIONS:
        ...
        apply_train_noise = True # Enable noise only when augmentations are on
    else:
        print("Augmentations DISABLED for training.")
        train_transforms = None
        apply_train_noise = False
    # ============================================================================
    val_transforms = None # Validation data is never augmented randomly

    # --- Create Datasets ---
    print("Creating Datasets...")
    try:
        # ============================================================================
        # MODIFIED: Pass conditional transform and noise variables
        # ============================================================================
        train_dataset = KelpDataset(
            train_sat, train_mask,
            transforms=train_transforms,       # Pass potentially None transforms
            apply_noise=apply_train_noise, # Pass conditional noise flag
            noise_level=0.01
        )
        val_dataset = KelpDataset(
            val_sat, val_mask,
            transforms=val_transforms, apply_noise=False # Validation always False
        )
        # ============================================================================
    ...
    if RUN_DIR.exists():
        ...
        if potential_checkpoints:
            ...
            resume_checkpoint_path = potential_checkpoints[0]; print(f"Run directory exists. Resuming from: {resume_checkpoint_path}")
    ...
    TRAIN_BATCH_SIZE = 8; VAL_BATCH_SIZE = 16 # Adjust based on GPU RAM for backbone
    NUM_WORKERS = psutil.cpu_count(logical=False)//2 if psutil else 0
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=TRAIN_BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=True if NUM_WORKERS > 0 else False, drop_last=True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=VAL_BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=True if NUM_WORKERS > 0 else False)

    # --- Initialize Model ---
    print("Initializing Model...")
    model = KelpSegmentationModel(target_size=(350, 350), learning_rate=1e-4, backbone_name=chosen_backbone)

    # --- Callbacks ---
    print("Configuring Callbacks...")
    checkpoint_callback = ModelCheckpoint(monitor='val_loss', dirpath=RUN_DIR, filename=f'{CHECKPOINT_BASENAME}-{{epoch:02d}}-{{val_loss:.4f}}', save_top_k=1, mode='min', save_last=False, verbose=True)
    early_stopping_callback = EarlyStopping(monitor='val_loss', mode='min', patience=EARLY_STOPPING_PATIENCE, min_delta=EARLY_STOPPING_MIN_DELTA, verbose=True)
